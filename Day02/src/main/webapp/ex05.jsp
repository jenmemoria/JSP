...
<!-- form 태그의 action 속성이 없으면,  [현재 주소]로 새로운 요청을 보낸다. -->
<form>
	<p>
...
<%
// 스크립틀릿 내부 코드는 servie() 함수 내부 내용이다.
	// 따라서, 지역변수는 함수가 호출될 때 새로 만들어진다. -> 값이 유지되지 않는다.
	// 리스트를 멤버 필드를 생성해두면, 객체가 소멸하기 전까지 (서버를 재시작하기 전까지) 값이 유지된다.
	
	// 4개의 파라미터를 전달받기
	String category = request.getParameter("category");	// <select name="category">라서
